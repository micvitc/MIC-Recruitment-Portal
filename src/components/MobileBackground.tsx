...
export default function MobileBackground({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden select-none bg-[#1093EB]">
      ...
      {/* Scattered Sky Clouds - Fixed behind content */}
      ...
      {/* ── PINNED Bottom Scene (Fixed at exact position above marquee footer) ── */}
      <div className="absolute bottom-[60px] left-0 w-full pointer-events-none z-0 overflow-hidden" style={{ height: "260px" }}>
        {/* City Skyline */}
      ...
      {/* ── PINNED Footer Marquee ── */}
      <div className="absolute bottom-0 left-0 w-full z-20 bg-[#DD9955] border-t-[4px] border-black overflow-hidden flex items-start pt-2 pointer-events-none" style={{ height: "60px" }}>
      ...
      {/* Main Content Area (Children) - Floating and scrolling above the pinned background */}
      <div className="relative z-10 w-full h-full flex flex-col overflow-y-auto overflow-x-hidden pb-[320px]">
        {children}
      </div>
    </div>
  );
